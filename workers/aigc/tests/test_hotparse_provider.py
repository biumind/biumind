...
def _cfg(**over) -> Config:
    env = {
        ...
        # 显式 env 模型 —— 本文件多数用例钉管线形状, 不依赖 preferred
        # 解析(该路径由 test_preferred_model_* 单独覆盖)。
        "AIGC_HOTPARSE_LLM_MODEL": "test.llm-model",
        "AIGC_HOTPARSE_STT_MODEL": "test.stt-model",
    }
    env.update(over)
...
@pytest.mark.asyncio
@respx.mock
async def test_preferred_model_resolved_and_cached() -> None:
    route = respx.get(RELAY + "/v1/internal/models/preferred").mock(
        return_value=httpx.Response(200, json={"code": "relay.preferred"}))
    p = HotparseProvider(_cfg(AIGC_HOTPARSE_LLM_MODEL=""),
                         client=httpx.AsyncClient())
    assert await p._preferred_model("chat", "AIGC_HOTPARSE_LLM_MODEL") == "relay.preferred"
    assert await p._preferred_model("chat", "AIGC_HOTPARSE_LLM_MODEL") == "relay.preferred"
    assert route.call_count == 1
    assert route.calls[0].request.url.params["mode"] == "chat"


@pytest.mark.asyncio
@respx.mock
async def test_preferred_model_404_fails_with_hint() -> None:
    respx.get(RELAY + "/v1/internal/models/preferred").mock(
        return_value=httpx.Response(404, text="no usable model"))
    p = HotparseProvider(_cfg(AIGC_HOTPARSE_STT_MODEL=""),
                         client=httpx.AsyncClient())
    with pytest.raises(ProviderError, match="AIGC_HOTPARSE_STT_MODEL"):
        await p._preferred_model("audio_transcription", "AIGC_HOTPARSE_STT_MODEL")
